...
void GroupableTimeSeriesSet::outlier(int length)
{
    if (length < 1 || length >= dataset->getSeqLength()) {
        session->geterr() << "Warning: Given invalid length: " << length << "." << endl;
        return;
    }

    if (grouping == NULL) {
        session->geterr() << "Warning: Attempted to find outliers on ungrouped dataset." << endl;
        return;
    }

    kBest best = grouping->getGroup(length)->getOutlier();
    if (best.seq == -1) {
        session->geterr() << "Warning: Failed to find outlier for length=" << length << "." << endl;
        return;
    }

    session->getout() << "Found outlier." << endl;
    session->getout() << "Sequence number and interval: " << best.seq << "@"
                      << "[" << best.interval.start << "," << best.interval.end << "]." << endl;
    session->getout() << "Total sum'd distance: " << best.dist << "." << endl;
}
...
enum _commands {
    ...
    _DEBUG,      // Set the verbosity level. {0:normal,1:debug,2:trace}
    _HELP,       // Print help.
...
void _print_help(ostream &out)
{
    ...
    out << "outlier <id> <l>     Find a dominant outlier in the dataset at length.   " << endl;
    out << endl;
    ...
    out << endl;
    out << "debug <value>        Set the verbosity. 0=normal, 1=debug, 2=trace.      " << endl;
